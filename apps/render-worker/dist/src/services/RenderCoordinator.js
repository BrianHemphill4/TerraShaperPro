...
class RenderCoordinator {
    ...
    initializeProviders() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.providerManager.initialize(configs);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return Sentry.startSpan({
                ...
                try {
                    ...
                    yield this.creditService.consumeCredits(organizationId, userId, renderId, settings);
                    yield job.updateProgress(10);
                    // Step 2: Initialize providers and prepare generation
                    yield this.initializeProviders();
                    const { generatedPrompt, generationOptions, provider } = yield this.prepareGeneration(prompt, settings, annotations);
                    yield job.updateProgress(30);
                    // Step 3: Generate image
                    const imageStartTime = Date.now();
                    const result = yield provider.generateImage(generatedPrompt.prompt, generationOptions);
                    Sentry.setMeasurement('image.generation', Date.now() - imageStartTime, 'millisecond');
                    yield job.updateProgress(70);
                    // Step 4: Quality control
                    const imageBuffer = yield this.storageService.prepareImageBuffer(result);
                    const { qualityResult, requiresManualReview } = yield this.qualityService.performQualityCheck(result.imageBase64 || result.imageUrl, generationOptions.width, generationOptions.height, renderId, settings, job.attemptsMade);
                    yield job.updateProgress(80);
                    // Step 5: Store results
                    const { imageUrl, thumbnailUrl } = yield this.storageService.storeRenderResult(renderId, projectId, imageBuffer, settings, generatedPrompt.hash);
                    yield job.updateProgress(95);
                    // Step 6: Update database and record usage
                    yield this.finalizeRender(renderId, imageUrl, thumbnailUrl, qualityResult, result.metadata, requiresManualReview);
                    yield this.creditService.recordUsage(organizationId, renderId, projectId, settings);
                    ...
                    yield this.handleRenderFailure(error, renderId, organizationId, userId, settings, startTime);
                    throw error;
                }
            }));
        });
    }
    prepareGeneration(prompt, settings, annotations) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const providerType = settings.provider === 'google-imagen'
                ? ai_service_1.RenderProvider.GOOGLE_IMAGEN
                : ai_service_1.RenderProvider.OPENAI_GPT_IMAGE;
            const provider = yield this.providerManager.getProvider(providerType);
            return { generatedPrompt, generationOptions, provider };
        });
    }
    finalizeRender(renderId, imageUrl, thumbnailUrl, qualityResult, metadata, requiresManualReview) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.supabase
                .from('renders')
                .update({
    ...
    handleRenderFailure(error, renderId, organizationId, userId, settings, startTime) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            ...
            if (error instanceof Error && error.message !== 'Insufficient credits') {
                yield this.creditService.refundCredits(organizationId, userId, renderId, settings, `Render failed: ${error.message}`);
            }
            yield this.qualityService.checkForFailurePatterns();
