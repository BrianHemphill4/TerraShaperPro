 ...
 */
export async function checkUsageLimit(ctx: Context, options: UsageLimitOptions): Promise<void> {
  if (!ctx.session) {
    throw new TRPCError({
      code: 'UNAUTHORIZED',
  ...
  const tier = (
    org?.stripe_subscription_status === 'active' ? org.subscription_tier : 'starter'
  ) as SubscriptionTier;
  ...
  switch (options.limitType) {
    ...
    case 'maxRendersPerMonth': {
      ...
      currentUsage = data?.reduce((sum: number, record: UsageRecord) => sum + record.count, 0) || 0;
      break;
    }

    case 'maxStorageGb': {
      // Get storage usage from helper function
      const { data } = await ctx.supabase.rpc('get_organization_storage_usage', {
