...
exports.storageRouter = (0, trpc_1.router)({
    ...
    generateUploadUrl: trpc_1.protectedProcedure.input(uploadRequestSchema).mutation(async ({ input }) => {
        ...
        try {
            const result = await storage_1.UploadUtils.generateDirectUploadUrl(bucketType, fileName, contentType, expiresInMinutes);
            return result;
        }
        catch {
            throw new server_1.TRPCError({
    ...
    generateDownloadUrl: trpc_1.protectedProcedure
        .input(fileOperationSchema.extend({
        ...
        try {
            ...
            const downloadUrl = await getStorageService().generateDownloadUrl(bucketType, fileName, expires);
            return {
                downloadUrl,
                fileName,
                expiresAt: expires,
    ...
    generateFileName: trpc_1.protectedProcedure
        ...
        switch (type) {
            ...
            case 'upload':
                ...
                fileName = storage_1.UploadUtils.generateUserUploadFileName(ctx.session.userId, originalFileName);
                break;
            default:
                throw new server_1.TRPCError({
                    code: 'BAD_REQUEST',
