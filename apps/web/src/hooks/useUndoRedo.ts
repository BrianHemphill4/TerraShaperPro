...
export const useUndoRedo = (
  ...
  const { maxHistorySize = 50, excludeFromHistory = (obj) => (obj as any).evented === false } = options;
  ...
  const saveState = useCallback(() => {
    ...
    const state = JSON.stringify({
      objects: objects.map((obj) => obj.toObject(['id', 'layerId', 'material', 'plantId', 'plantName'])),
  ...
  const loadState = useCallback((state: HistoryState) => {
    if (!canvas) return;
