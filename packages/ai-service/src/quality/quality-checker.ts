...
export class QualityChecker {
  ...
  async checkQuality(
    ...
    if (size > thresholds.maxFileSize) {
      issues.push(
        `File size ${(size / 1024 / 1024).toFixed(2)}MB exceeds maximum ${(thresholds.maxFileSize / 1024 / 1024).toFixed(2)}MB`
      );
      score -= 0.2;
    }
    ...
    if (
      resolution.width < thresholds.minResolution.width ||
      resolution.height < thresholds.minResolution.height
    ) {
      issues.push(
        `Resolution ${resolution.width}x${resolution.height} below minimum ${thresholds.minResolution.width}x${thresholds.minResolution.height}`
      );
      score -= 0.3;
    }
    ...
    if (qualityScore < thresholds.minQualityScore) {
      issues.push(
        `Quality score ${qualityScore.toFixed(2)} below threshold ${thresholds.minQualityScore}`
      );
      score -= 0.2;
    }
