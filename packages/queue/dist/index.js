...
__export(index_exports, {
  ...
  canUserSubmitRender: () => canUserSubmitRender,
  closeRenderQueue: () => closeRenderQueue,
  createRenderWorker: () => createRenderWorker,
  defaultQueueOptions: () => defaultQueueOptions,
  getQueueEventEmitter: () => getQueueEventEmitter,
  getQueueMetrics: () => getQueueMetrics,
  getRenderJobCount: () => getRenderJobCount,
  getRenderQueue: () => getRenderQueue,
  getRenderQueueEvents: () => getRenderQueueEvents,
...
function getRenderQueue(connection) {
  if (!renderQueue) {
    renderQueue = new import_bullmq.Queue(
      QUEUE_NAMES.RENDER,
      __spreadValues(__spreadValues({}, defaultQueueOptions), connection && { connection })
    );
  }
  return renderQueue;
}
function getRenderQueueEvents(connection) {
  if (!renderQueueEvents) {
    renderQueueEvents = new import_bullmq.QueueEvents(QUEUE_NAMES.RENDER, {
...
function addRenderJob(data, options) {
  return __async(this, null, function* () {
    ...
    const priority =
      (_a = options == null ? void 0 : options.priority) != null
        ? _a
        : QUEUE_PRIORITIES[data.subscriptionTier.toUpperCase()];
...
function createRenderWorker(processFunction, connection, concurrency = 5) {
  return new import_bullmq.Worker(QUEUE_NAMES.RENDER, processFunction, {
    connection: connection || defaultQueueOptions.connection,
    concurrency,
    autorun: true,
  });
...
var QueueEventEmitter = class extends import_events.EventEmitter {
  constructor() {
    super();
