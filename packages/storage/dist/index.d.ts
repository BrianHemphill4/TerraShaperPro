...
declare class ImageProcessor {
    static optimizeImage(buffer: Buffer, options?: ImageOptimizationOptions): Promise<{
        buffer: Buffer;
        contentType: string;
        size: number;
    }>;
...
declare class StorageService {
    ...
    uploadImage(bucketType: StorageBucket, fileName: string, buffer: Buffer, optimizationOptions?: ImageOptimizationOptions): Promise<{
        original: UploadResult;
        thumbnail?: UploadResult;
    }>;
    generateSignedUrl(options: SignedUrlOptions): Promise<string>;
    generateUploadUrl(bucketType: StorageBucket, fileName: string, contentType: string, expires?: Date): Promise<string>;
    ...
    getFileMetadata(bucketType: StorageBucket, fileName: string): Promise<_google_cloud_storage.FileMetadata>;
    copyFile(sourceBucket: StorageBucket, sourceFileName: string, destBucket: StorageBucket, destFileName: string): Promise<void>;
    getPublicUrl(bucketName: string, fileName: string): string;
    private getThumbnailFileName;
    uploadRenderResult(renderId: string, imageBuffer: Buffer, metadata?: Record<string, string>): Promise<{
        original: UploadResult;
        thumbnail: UploadResult;
    }>;
    uploadAsset(assetId: string, imageBuffer: Buffer, metadata?: Record<string, string>): Promise<{
        original: UploadResult;
        thumbnail: UploadResult;
    }>;
}
...
declare class RenderStorageService extends StorageService {
     ...
     */
    storeRenderResult(renderId: string, imageBuffer: Buffer, metadata: {
        userId: string;
        projectId: string;
        sceneId: string;
        provider: string;
     ...
     */
    generateRenderDownloadUrls(renderId: string, expiresInHours?: number): Promise<{
        original?: string;
        webp?: string;
        thumbnail?: string;
    }>;
...
declare class UploadUtils {
     ...
     */
    static generateDirectUploadUrl(bucketType: StorageBucket, fileName: string, contentType: string, expiresInMinutes?: number): Promise<{
        uploadUrl: string;
        fileName: string;
        publicUrl: string;
        expiresAt: Date;
     ...
     */
    static validateImageFile(contentType: string, _maxSizeMB?: number): {
        valid: boolean;
        error?: string;
    };
    /**
     ...
     */
    static generatePresignedPost(bucketType: StorageBucket, fileName: string, contentType: string, _maxSizeBytes?: number): Promise<{
        url: string;
        fields: Record<string, string>;
    }>;
}

export { type ImageOptimizationOptions, ImageProcessor, RenderStorageService, type SignedUrlOptions, type StorageBucket, type StorageConfig, StorageService, type UploadOptions, type UploadResult, UploadUtils, StorageService as default, getBucket, getStorageClient, getStorageConfig };
